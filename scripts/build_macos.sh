...
CPPEXP="-lstdc++exp"
SRC_CORE="main.cpp base_lib.cpp platform_macos.cpp input.cpp gl_renderer.cpp"
SRC_DLLS=""
SRC_LIBS=""
SRC_PCHS=""
FILE_EXT=""
DLL_FILE_NAME="shared_lib.dylib"
LIB_FILE_NAME="static_lib.a"
LINKER_LIBS="-framework OpenGL"
EXE_FILE_NAME="CelesteClone"
...
BIN_DIR="$PROJECT_DIR/bin"          ## <-- For exe, dylib, lib, pch output
...
GXX=$(which clang++)
LD_PATTERN=$(clang++ -print-prog-name=ld | sed 's|/|\\/|g')
...
link_target() {
    ...
    case "$mode" in
        ...
        dll)
            ...
            eval "$GXX -dynamiclib -o \"$BIN_DIR/$target\" $objs $LINKER_LIBS $CPPEXP" 2>&1 | sed -E \
                -e "s|$LD_PATTERN:||g" \
                -e "s|[^[:space:]]*\.o:||g" \
                -e "s|$SOURCE_DIR/||g" \
                -e 's|\(.[^[:space:]]*[]]|(|'
...
for arg in "$@"; do
    case "$arg" in
        ...
        *.cpp | *.hpp)
            if [[ "$arg" == *.hpp ]]; then
                PCHD_BUILD+=" $(find "$SOURCE_DIR" -name "$arg")"
            else
...
if [[ $AUTO -eq 1 ]]; then
    ...
    PCHD_BUILD=$(find "$SOURCE_DIR" -name '*.hpp')
fi
...
for hdr in "${PCH_HDRS[@]}"; do
    link_target pch "$(basename "${hdr%.hpp}")" "$hdr"
done
