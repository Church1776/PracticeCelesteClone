...
SRC_FILES="main.cpp main_game_lib.cpp platform_win32.cpp input.cpp gl_renderer.cpp"
SRC_DLLS=""
LINKER_LIBS="-lGL -lGLU -lX11"       ## Linux OpenGL + X11
EXECUTABLE="CelesteClone"
DLL_FILE_NAME=""
...
BIN_DIR="$PROJECT_DIR/bin"          ## <-- For executable and shared libraries
...
if [[ $AUTO -eq 1 ]]; then
    ...
    if [[ DLL_MODE -eq 1 ]]; then
        SHRD_BUILD=""
        SHRD_BUILD=$(find "$SOURCE_DIR" -name '*.cpp')
    else
        SRC_BUILD=""
        SRC_BUILD=$(find "$SOURCE_DIR" -name '*.cpp')
    fi
fi
if [[ -z $SRC_BUILD && -z $SHRD_BUILD ]]; then
...
for src in $SRC_BUILD; do    
    ...
    obj="$BUILD_DIR/source/${rel_path%.cpp}.o"
    mkdir -p "$(dirname "$obj")"
...
for dll in $SHRD_BUILD; do    
    ...
    obj="$BUILD_DIR/shared/${rel_path%.cpp}.o"
    mkdir -p "$(dirname "$obj")"
    "$GXX" $CPP_VERSION $INCLUDE_FLAGS -c $DEBUG_FLAGS "$dll" -fPIC -o "$obj" 2>&1 | sed -E \
        -e 's|/[^[:space:]]*[/]||g' \
        -e 's/candidate:/\n candidate:/g' \
        -e 's|^[[:space:]]*||'
    if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
...
if [[ $DLL_MODE -eq 1 ]]; then
    ...
    for subdir in "$BUILD_DIR"/shared/*/; do
        ...
        DLL_NAME="$(basename "$subdir").so"
        OBJ_FILES=("$subdir"/*.o)

        mkdir -p "$BIN_DIR"
        LINK_DLL="$GXX -shared -fPIC -o \"$BIN_DIR/$DLL_NAME\" ${OBJ_FILES[*]} $LINKER_LIBS $CPPEXP"
